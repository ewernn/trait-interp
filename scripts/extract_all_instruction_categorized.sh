#!/bin/bash
# Full instruction-based extraction for all 19 traits (CATEGORIZED STRUCTURE)
...
echo "================================================================" | tee -a "$LOG_FILE"
echo "INSTRUCTION-BASED EXTRACTION - 19 TRAITS (CATEGORIZED)" | tee -a "$LOG_FILE"
echo "================================================================" | tee -a "$LOG_FILE"
...
for trait in "${ALL_TRAITS[@]}"; do
  category=$(get_category "$trait")
  trait_path=$(get_trait_path "$EXPERIMENT" "$trait")
...
if [ $COMPLETED -eq 19 ]; then
  echo "✅ SUCCESS: All 19 traits extracted!" | tee -a "$LOG_FILE"
elif [ $COMPLETED -ge 14 ]; then
  echo "⚠️  PARTIAL SUCCESS: $COMPLETED/19 traits extracted" | tee -a "$LOG_FILE"
else
  echo "❌ FAILURE: Only $COMPLETED/19 traits extracted" | tee -a "$LOG_FILE"
fi

echo "" | tee -a "$LOG_FILE"
echo "Verify results:" | tee -a "$LOG_FILE"
echo "  find experiments/$EXPERIMENT/*/*/extraction/vectors -name '*.pt' | wc -l" | tee -a "$LOG_FILE"
